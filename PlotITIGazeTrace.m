function PlotITIGazeTrace(NextTrial,CurrentTrial, TrialEnter, TrialExit, dx, dy, fractal_win, fixation_x, fixation_y, fixation_w, fixation_h)
    ...
    indices_in_range = find(NextTrial.time >= TrialEnter & NextTrial.time <= TrialExit);

    % Extract the gx and gy values in that time range
    gx_in_range = NextTrial.gx(indices_in_range);
    gy_in_range = NextTrial.gy(indices_in_range);
    time_in_range = NextTrial.time(indices_in_range);
    ...
    hold off;
    display(max(time_in_range)-min(time_in_range));
end
